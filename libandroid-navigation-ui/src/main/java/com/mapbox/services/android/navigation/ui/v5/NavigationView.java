...
public class NavigationView extends CoordinatorLayout implements LifecycleOwner, OnMapReadyCallback,
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        wayNameView.setWaynameText(navigationViewInstanceState.getWayNameText());
    ...
    public void updateWaynameVisibility(boolean isVisible) {
        if (navigationMap != null) {
            if (isVisible) {
                wayNameView.setVisibility(View.VISIBLE);
            } else {
                wayNameView.setVisibility(View.INVISIBLE);
            }
        }
    }
    ...
    private void initializeNavigationViewModel() {
        try {
            navigationViewModel = ViewModelProviders.of(ContextHelper.getFragmentActivity(getContext())).get(NavigationViewModel.class);
     ...
     * method calls based on the {@link LiveData} updates.
     */
    private void subscribeViewModels() {
        instructionView.subscribe(this, navigationViewModel);
        summaryBottomSheet.subscribe(this, navigationViewModel);

        NavigationViewSubscriber subscriber = new NavigationViewSubscriber(this, navigationPresenter, navigationViewModel);
